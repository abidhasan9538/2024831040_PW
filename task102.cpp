// Task_102: Expanding circle that resets when it touches any window edge.
// Only dependency: SDL2/SDL.h
// Build: g++ task_102.cpp -o task_102 $(sdl2-config --cflags --libs)
...
// ── Filled circle via midpoint scan-line algorithm ─────────────────────────
void drawFilledCircle(SDL_Renderer* renderer, int cx, int cy, int r)
{
    int x = 0, y = r, d = 1 - r;

    // Lambda: draw one horizontal span
...
// ── Circle outline via midpoint algorithm ──────────────────────────────────
...
int main()
{
    ...
    while (running)
    {
        ...
        // Collision: circle edge reaches any wall → reset
